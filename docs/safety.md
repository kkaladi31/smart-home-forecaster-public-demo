...
| R15 | **Answers about the wrong location after the user corrects it** | The user says "actually I'm in X, not the saved address" mid-conversation, and the agent keeps using the saved home | The active home rides the run config from the **UI home switcher**, which is authoritative and cannot be overridden by chat text — so the documented, supported way to change homes is a control, not a sentence. A stated correction is additionally honoured by a dedicated prompt rule placed beside the weather rules. **Measured residual risk: on the free model that rule holds in roughly 1 run in 4** (eval A13, left deliberately failing rather than skipped). It holds consistently on `nvidia/nemotron-3-super-120b-a12b:free`. Consequence is a forecast for a property the user is not in, so the answer always names the resolved address it used — the user can see the mismatch | `agents/orchestrator.py: SYSTEM_PROMPT`, `tools/homes.py: current_home_id`, eval **A13** |
